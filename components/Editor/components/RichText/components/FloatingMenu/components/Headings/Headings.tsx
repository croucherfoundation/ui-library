...
const Headings = () => {
  const { headings, selectedHeading, handleChangeHeading } = useContainer();

  return (
    <div className="w-fit">
      <Listbox value={selectedHeading}>
                ...
                <Listbox.Option
                  ...
                  className={() =>
                    `relative cursor-pointer py-2 text-center hover:bg-edt-secondary ${
                      selectedHeading.value === heading.value
                        ? "bg-edt-secondary"
                        : ""
                    }`
                  }
                  onClick={() => handleChangeHeading(heading)}
                  value={heading.value}
                >
                  {() => (
                    <>
                      <span className="capitalize text-sm">{heading.name}</span>
                    </>
                  )}
                </Listbox.Option>
              ))}
